...
const contentSecurityPolicy = `
  ...
  script-src 'self' https://matomo.incubateur.anct.gouv.fr https://client.crisp.chat 'unsafe-inline' 'unsafe-eval';
  ...
  connect-src 'self' ${
    ServerWebAppConfig.S3.uploadsBucket
      ? `https://${ServerWebAppConfig.S3.uploadsBucket}.${ServerWebAppConfig.S3.host}`
      : ''
  ...
  frame-ancestors 'self' http://localhost:* *.gouv.fr;
  form-action 'self';
  base-uri 'self';
